...
public class Fragmento3 extends Fragment {
    ...
    private void actualizar() {
        try{
            ...
            String SQL = "UPDATE TAREA SET DESCRIPCION='{DESC}', FECHACREACION='{FECHAC}', FECHAENTREGA='{FECHAE}', MATERIA='{MATERIA}' "
                    +"WHERE ID="+idBuscar.getText().toString();

            SQL = SQL.replace("{DESC}", descripcion.getText().toString());
            SQL = SQL.replace("{FECHAC}", fechacreacion.getText().toString());
            SQL = SQL.replace("{FECHAE}", fechaentrega.getText().toString());
            SQL = SQL.replace("{MATERIA}", materia.getSelectedItem().toString());

            db.execSQL(SQL);

            Toast.makeText(getContext(), "SE REEMPLAZO SATISFACTORIAMENTE", Toast.LENGTH_LONG).show();
            descripcion.setText("");
    ...
    private void buscarID(String valorABuscar) {
        try{
            ...
            String materia = x.getString(4);
            ArrayAdapter adaptador = (ArrayAdapter) this.materia.getAdapter();
            int posicion = adaptador.getPosition(materia);
            this.materia.setSelection(posicion);
            ...
            this.materia.setEnabled(true);

            base.close();
        }catch (SQLiteException e){
            AlertDialog.Builder alert = new AlertDialog.Builder(getContext());
            alert.setTitle("ERROR PAPÁ:").setMessage(e.getMessage()).show();
        }
    }//fin buscarID
